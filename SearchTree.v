...
Definition combine {A} (pivot: key) (m1 m2: total_map A) : total_map A :=
  fun x => match x with Id x' => if x' <? pivot  then m1 x else m2 x end.
...
Fixpoint list2map (el: list (key*V)) : total_map V :=
 match el with
 | nil => t_empty default
 | (i,v)::el' => t_update (list2map el') (Id i) v
 end.

(* EX 3 (elements_relate_informal) *)
...
pose (m := t_update (t_empty default) (Id 2) v).
pose (m' := t_update
        (combine 2
           (t_update (combine 3 (t_empty default) (t_empty default)) (Id 3) v)
           (t_empty default)) (Id 2) v).
...
Lemma list2map_app_right:
  forall (al bl: list (key*V)) (i: key),
     ~(exists v, In (i,v) al) -> list2map (al++bl) (Id i) = list2map bl (Id i).
Proof.
intros.
revert H; induction al as [| [j w] al]; intro; simpl; auto.
unfold t_update, beq_id.
...
Lemma list2map_not_in_default:
 forall (al: list (key*V)) (i: key),
   ~(exists v, In (i,v) al) -> list2map al (Id i) = default.
Proof.
intros.
...
Lemma lookup_relate':
  forall (k : key) (t : tree) (cts : total_map V),
     SearchTree t -> Abs t cts -> lookup k t = cts (Id k).
...
intros.
evar (m: total_map V).
replace  (t_update (t_empty default) (Id 2) v2) with m; subst m.
repeat constructor.
extensionality x; destruct x as [x].
unfold t_update, combine, t_empty, beq_id.
bdestruct (2 =? x).
auto.
bdestruct (x <? 2).
bdestruct (3 =? x).
...
Theorem naive_lookup_relateX:
  forall k t cts ,
    AbsX t cts -> lookup k t =  cts (Id k).
Abort.  (* Not true *)
...
Theorem not_naive_lookup_relateX:
   forall v, default <> v ->
    ~ (forall k t cts , AbsX t cts -> lookup k t =  cts (Id k)).
Proof.
unfold AbsX.
intros v H.
intros H0.
pose (bogus := T (T E 3 v E) 2 v E).
pose (m := t_update (t_update (t_empty default) (Id 2) v) (Id 3) v).
assert (list2map (elements bogus) = m).
  reflexivity.
assert (~ lookup 3 bogus = m (Id 3)). {
...
Theorem lookup_relateX:
  forall k t cts ,
    SearchTree t -> AbsX t cts -> lookup k t =  cts (Id k).
Proof.
intros.
